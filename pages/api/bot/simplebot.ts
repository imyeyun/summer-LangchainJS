...
//LLM응답 메시지 타입을 원하는 타입결과물로 파싱(변환)해주는 아웃풋 파서 참조하기
//StringOutputParser는 AIMessage타입에서 content 속성값만 문자열로 제공
...
export default async function handler(
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      //Case1: OpenAI llm 모델 기반 질의응답 처리하기
      // const result = await llm.invoke(prompt);

      //Case2 : 메시지 객체를 이용해서 llm 연동하기 - PromptTemplate1
      //SystemMessage 객체는 LLM의 역할이나 질문에 관련된 주요정보를 LLM에게 전달하는 역할제공
      //HumanMessage 객체는 사용자 보낸 질문 메시지를 저장해서 llm에 전달가능한 객체 생성
      // const messages = [
      ...
      //Case3: ChatPromptTemplate을 이용한 프롬프트 전달하기
      //프롬프트 템플릿이란? LLM에게 전달할 수 있는 다양한 질문템플릿을 제공하여 보다 효율적인 질문형식을
      ...
      //CASE4: LLM 응답결과 메시지는 기본 AIMessage 객체를 반환하지만
      ...
      //template.pipe().pipe(): 두개의 체인을 만들고 순차적으로 두개의 체인목록을 가진 결과체인 반환
      const chains = PromptTemplate.pipe(llm).pipe(outputParser);
      //outputParser로 인해 result 값은 실제 llm의 응답메시지 문자열이 반환됨(AIMessage.content)
      ...
      //메시지 처리결과데이터: result가 AIMessage 타입인경우(CASE1~3에 해당하는 경우만)
      // const resultMsg: IMessage = {
      //   user_type: UserType.BOT,
